...
export const useCrudProductContext = () => { return useContext(CrudProductContext) }
...
const CrudProvider = ({ children }: CrudProviderProps) => {
    ...
    const crudProduct_default: CrudProduct = { id: 0, name: '', description: '', category: '', price: 0, rating: 0 }
    const crudProductRequest_default: CrudProductRequest = { name: '', description: '', category: '', price: 0, rating: 0 }
    ...
    const getAll = async () => {
        const result = await fetch(`${baseUrl}`)
        if (result.status === 200)
            setCrudProducts(await result.json())
    }
    ...
    return (
        <CrudProductContext.Provider value={{ crudProduct, setCrudProduct, crudProductRequest, setCrudProductRequest, crudProducts, create, get, getAll, update, remove }}>
            {children}
