...
const MongoProductRankingSection: React.FC<ProductCardType> = ({ title1, title2, title3, itemsOne = [], itemsTwo = [], itemsThree = [] }) => {
        ...
        <section className="container-products-category-ranking">
            <div className="container">
                <div className="latest">
                    <h1>{title1}</h1>
                    {
                        itemsOne.map(product => <MongoProductCardComponent key={product.articleNumber} item={product} />)
                    }
                </div>
                <div className="best">
                    <h1>{title2}</h1>
                    {
                        itemsTwo.map(product => <MongoProductCardComponent key={product.articleNumber} item={product} />)
                    }
                </div>
                <div className="reacted">
                    <h1>{title3}</h1>
                    {
                        itemsThree.map(product => <MongoProductCardComponent key={product.articleNumber} item={product} />)
                    }
                </div>
            </div>
...
export default MongoProductRankingSection;
